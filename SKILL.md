...
Six stages. Intake sets the depth: a one-paragraph request gets one pass through all six in a few seconds of thought, a corpus for a board decision gets days. Do not skip a stage. A skipped stage becomes a defect at a later and more expensive point.
...
When the artifact fails, the failure names the stage that produced it. Re-enter there. Do not repair a structural failure with sentence edits.
...
Select; do not compress. Cut a unit only when it does not improve understanding, judgment, memory, action, retrieval, or the requested experience. When a unit survives, name why. Risk aversion and sunk production cost are not benefits the audience receives.
...
**No story is available.** The material has no actor, no change, no chronology. Do not manufacture one. Produce the map, explanation, comparison, or reference, and state in the response that the material does not support a narrative.
...
**The material is adversarial or the audience is hostile.** Lead with what the audience already believes and can verify. Make the evidence independently inspectable before the conclusion, not after. Do not use narrative transport on an audience that is arguing as it reads.
